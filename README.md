...
> ⚠️ 当前开发库默认使用 **MySQL `housing`**，数据来自已导入的山东 `house_info.tsv` 与链家快照数据；`backend/seed.py` 仅作为合成演示数据脚本，当前不建议用于正式演示数据。
...
| 层 | 技术 |
|----|------|
| 后端 | Flask 3 · Flask-SQLAlchemy · Flask-CORS · scikit-learn · pandas |
...
│   ├── models/             # ORM 模型：城市/区/房源/设施/价格走势
│   ├── api/                # 蓝图路由：cities/districts/properties/stats/facilities
...
└── frontend/               # Vue 3 前端
    ...
    └── src/
        ├── views/          # Dashboard / Explore / PropertyDetail / Analysis
        ├── components/     # CityMap3D（Three.js）/ 图表 / 预测表单 / 卡片
        ├── api/            # axios 封装与接口定义
        ├── store/          # Pinia 状态
        └── router/
```
...
| GET | `/api/properties/{id}` | 房源详情（含周边设施与山东交易属性） |
