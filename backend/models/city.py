...
class City(db.Model):  # 逐行注释：声明类并定义相关数据或行为。
    """表示城市基础信息及其下属行政区关系。"""
    __tablename__ = "cities"  # 逐行注释：赋值或更新当前变量/字段。
    ...
    province = db.Column(db.String(50))  # 逐行注释：赋值或更新当前变量/字段。
    lng = db.Column(db.Float)  # 中心经度
    lat = db.Column(db.Float)  # 中心纬度

    districts = db.relationship(  # 逐行注释：赋值或更新当前变量/字段。
        "District", back_populates="city", cascade="all, delete-orphan"  # 逐行注释：赋值或更新当前变量/字段。
    )  # 逐行注释：结束当前数据结构或调用块。
    ...
    def to_dict(self) -> dict:  # 逐行注释：声明函数或方法入口。
        ...
        return {  # 逐行注释：返回当前逻辑的处理结果。
            ...
            "name_en": self.name_en,  # 逐行注释：设置当前数据项或参数。
            "province": self.province,  # 逐行注释：设置当前数据项或参数。
            "lng": self.lng,  # 逐行注释：设置当前数据项或参数。
            "lat": self.lat,  # 逐行注释：设置当前数据项或参数。
            "district_count": len(self.districts),  # 逐行注释：设置当前数据项或参数。
        }  # 逐行注释：结束当前数据结构或调用块。
